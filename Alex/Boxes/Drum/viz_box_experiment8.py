"""At the moment, the program can be run for up to 20 trial runs. This does not mean that the notes can hold a max of 20 
trails. It just means that after 20 trials, we will have to restart the program. We will not truncate the data (unless we wish
to, in which case we can manually), so all the data will just continue."""
...
#tracking data
#the hand module is in charge of the creation, movement, and tracking of the hand. 
import hand
...
def printData():
	...
	print ('Hand Tracker position',handTracker.getPosition())
	print ('Hand Tracker euler',handTracker.getEuler()) 
...
#these are the parent nodes that the proximity sensors will use. 
...
start_node= vizshape.addBox([.75,.25,.75],right=True,left=True,top=True,bottom=True, front=True,back=True, splitFaces=False, color=viz.BLUE)

#these are the visual planes. We will create an amound dependant on how many trials we will have. In other wors, how long
#the control array (or the valus of global x) is. Each plane has dimensions dependant on its respective value in the array.
#Each plane is also shifted by 50, as it moved to the left by 50 after each trial in order to change planes. 
if x>0:
...
#manager for proximity sensors
manager = vizproximity.Manager()		
manager.setDebug(True)

#sensor_parameters= [scale_x1,height1,scale_z1]
#parameters for start sensor:
start_parameters= [scale_x1,1,scale_z1]
#initial translations to make everything work:
right_node1.translate(0,500,0)
start_node.translate(x_translate1,y_height1,z_depth1)
#here we create our sensors. the amount we create is, once again, dependant on how many trials we have. Just like our planes,
#we distance the sensors by 50 units, attatch them all to the same parent node, and have them all move together. This way,
#as we go through a trial, we fix everything to the right. Imagine eating across a sish kebab or corn on the cob. 
if x>0:
	#first we create error sensors. At the moment, error sensors have slightly enlarged (+.25) x and z dimensions. We can change 
	#this depending on what we want to count as the error. 
	...
	#Now we create the left and right plane sensors. 
...
#the process repeats until the conditions become false. The larger the length of the control array, the more instances of error
#and main sensors. I couldn't create a recurssive loop because of so many variables that have to be independantly named and 
#created. 
if x>1:
...
if x>12:
	...
	left_plane_sensor_error13=vizproximity.Sensor(vizproximity.Box([control_array[12]+.25,.5,control_array[12]+.25],[-x_translate1+600,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor_error13=vizproximity.Sensor(vizproximity.Box([control_array[12]+.25,.5,control_array[12]+.25],[x_translate1+600,y_height1,z_depth1]),right_node1)
	...
	left_plane_sensor13= vizproximity.Sensor(vizproximity.Box([control_array[12],.25,control_array[12]],[-x_translate1+600,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor13= vizproximity.Sensor(vizproximity.Box([control_array[12],.25,control_array[12]],[x_translate1+600,y_height1,z_depth1]),right_node1)
...
if x>13:
	...
	left_plane_sensor_error14=vizproximity.Sensor(vizproximity.Box([control_array[13]+.25,.5,control_array[13]+.25],[-x_translate1+650,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor_error14=vizproximity.Sensor(vizproximity.Box([control_array[13]+.25,.5,control_array[13]+.25],[x_translate1+650,y_height1,z_depth1]),right_node1)
	...
	left_plane_sensor14= vizproximity.Sensor(vizproximity.Box([control_array[13],.25,control_array[13]],[-x_translate1+650,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor14= vizproximity.Sensor(vizproximity.Box([control_array[13],.25,control_array[13]],[x_translate1+650,y_height1,z_depth1]),right_node1)
...
if x>14:
	...
	left_plane_sensor_error15=vizproximity.Sensor(vizproximity.Box([control_array[14]+.25,.5,control_array[14]+.25],[-x_translate1+700,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor_error15=vizproximity.Sensor(vizproximity.Box([control_array[14]+.25,.5,control_array[14]+.25],[x_translate1+700,y_height1,z_depth1]),right_node1)
	...
	left_plane_sensor15= vizproximity.Sensor(vizproximity.Box([control_array[14],.25,control_array[14]],[-x_translate1+700,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor15= vizproximity.Sensor(vizproximity.Box([control_array[14],.25,control_array[14]],[x_translate1+700,y_height1,z_depth1]),right_node1)
...
if x>15:
	...
	left_plane_sensor_error16=vizproximity.Sensor(vizproximity.Box([control_array[15]+.25,.5,control_array[15]+.25],[-x_translate1+750,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor_error16=vizproximity.Sensor(vizproximity.Box([control_array[15]+.25,.5,control_array[15]+.25],[x_translate1+750,y_height1,z_depth1]),right_node1)
	...
	left_plane_sensor16= vizproximity.Sensor(vizproximity.Box([control_array[15],.25,control_array[15]],[-x_translate1+750,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor16= vizproximity.Sensor(vizproximity.Box([control_array[15],.25,control_array[15]],[x_translate1+750,y_height1,z_depth1]),right_node1)
...
if x>16:
	...
	left_plane_sensor_error17=vizproximity.Sensor(vizproximity.Box([control_array[16]+.25,.5,control_array[16]+.25],[-x_translate1+800,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor_error17=vizproximity.Sensor(vizproximity.Box([control_array[16]+.25,.5,control_array[16]+.25],[x_translate1+800,y_height1,z_depth1]),right_node1)
	...
	left_plane_sensor17= vizproximity.Sensor(vizproximity.Box([control_array[16],.25,control_array[16]],[-x_translate1+800,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor17= vizproximity.Sensor(vizproximity.Box([control_array[16],.25,control_array[16]],[x_translate1+800,y_height1,z_depth1]),right_node1)
...
if x>17:
	...
	left_plane_sensor_error18=vizproximity.Sensor(vizproximity.Box([control_array[17]+.25,.5,control_array[17]+.25],[-x_translate1+850,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor_error18=vizproximity.Sensor(vizproximity.Box([control_array[17]+.25,.5,control_array[17]+.25],[x_translate1+850,y_height1,z_depth1]),right_node1)
	...
	left_plane_sensor18= vizproximity.Sensor(vizproximity.Box([control_array[17],.25,control_array[17]],[-x_translate1+850,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor18= vizproximity.Sensor(vizproximity.Box([control_array[17],.25,control_array[17]],[x_translate1+850,y_height1,z_depth1]),right_node1)
...
if x>18:
	...
	left_plane_sensor_error19=vizproximity.Sensor(vizproximity.Box([control_array[18]+.25,.5,control_array[18]+.25],[-x_translate1+900,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor_error19=vizproximity.Sensor(vizproximity.Box([control_array[18]+.25,.5,control_array[18]+.25],[x_translate1+900,y_height1,z_depth1]),right_node1)
	...
	left_plane_sensor19= vizproximity.Sensor(vizproximity.Box([control_array[18],.25,control_array[18]],[-x_translate1+900,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor19= vizproximity.Sensor(vizproximity.Box([control_array[18],.25,control_array[18]],[x_translate1+900,y_height1,z_depth1]),right_node1)
...
if x>19:
	...
	left_plane_sensor_error20=vizproximity.Sensor(vizproximity.Box([control_array[19]+.25,.5,control_array[19]+.25],[-x_translate1+950,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor_error20=vizproximity.Sensor(vizproximity.Box([control_array[19]+.25,.5,control_array[19]+.25],[x_translate1+950,y_height1,z_depth1]),right_node1)
	...
	left_plane_sensor20= vizproximity.Sensor(vizproximity.Box([control_array[19],.25,control_array[19]],[-x_translate1+950,y_height1,z_depth1]),left_node1)
	...
	right_plane_sensor20= vizproximity.Sensor(vizproximity.Box([control_array[20],.25,control_array[20]],[x_translate1+900,y_height1,z_depth1]),right_node1)
...
#next we create other sensors. 
#the mid sensor will be used to see how many times we cross the mid plane.
...
#the start_sensor will be used when we enter the start box.     
start_sensor=vizproximity.Sensor(vizproximity.Box([.75,.25,.75],center=[0,0,0]),start_node)
manager.addSensor(start_sensor)

#the end sensor will be used when we enter the end box. 
...
#the timer3 function is used primarily for the delayed start time when a subject enters the start box.     
def timer3():
	viz.playSound('boing!.wav')
...
#when someone enters the start sensor, we make the various manipulations to make the planes change. 	
...
#all the data is recorded and written when the subject enters the end sensor. 
def EnterProximity_end_sensor(end_sensor):
	...
	print ('right_error:'+ str(right_error))
	...
	print(b)
	start_node.translate(x_translate1,y_height1,z_depth1)
	#when we leave the box sensor, we need to move the box sensor away. 
...
#the target is the object that enters the proximity sensors and triggers the proximity events. 
