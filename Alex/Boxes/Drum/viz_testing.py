...
x_translate_p1=x_translate+1.5
height=.025
#tracking data
#import hand
...
start_node= vizshape.addCylinder(height, radius=.00000000005,topRadius= None,bottomRadius= None, axis=vizshape.AXIS_Y,slices=20,bottom=True,top=True)

left_node=vizshape.addCylinder(height, radius= .000001,topRadius= None,bottomRadius= None, axis=vizshape.AXIS_Y,slices=20,bottom=True,top=True)        
right_node=vizshape.addCylinder(height, radius=.00000001,topRadius= None,bottomRadius= None, axis=vizshape.AXIS_Y,slices=20,bottom=True,top=True)
...
start_parameters= [scale_x,1,scale_z]
left_node.translate(0,500,0)
start_node.translate(x_translate,y_height,z_depth)
...
start_sensor=vizproximity.Sensor(vizproximity.Box([scale_x,1,scale_y],center=[0,0,0]),start_node)
manager.addSensor(start_sensor)

end_sensor=vizproximity.Sensor(vizproximity.Box([scale_x,1,scale_y],center= [x_translate_p1, y_height,z_depth]),left_node)
...
def EnterProximity_start_sensor(start_sensor):
	vizact.ontimer2(3,0,timer3)
	print('Entered start_sensor')
	
def EnterProximity_end_sensor(end_sensor):
	start_node.translate(x_translate,y_height,z_depth)
	left_node.setPosition(0,100,0)
	print('Entered end_sensor')
def ExitProximity_end_sensor(end_sensor):
	print('Exited end_sensor at:', viz.tick())
...
def ExitProximity_start_sensor(start_sensor):
	...
	global timer1
	timer1=viz.tick()
	left_node.setPosition(0,0,0)
	print('Exited start_sensor at:')
