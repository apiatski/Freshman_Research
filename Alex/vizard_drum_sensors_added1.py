...
import vizuniverse
import vizproximity
import VizardHMDVideoVision_Alex1 as viztracker
from VizardHMDVideoVision_Alex1 import *
...
if numhands >= 1:
	...
	LeftSensor = viz.addGroup(pos=[0,-10,0]) # Create tracker that is below the room
...
kick_bass_sensor= vizproximity.Sensor(vizproximity.Box([.5,.5,.5],center=[0,.5,0]),source=  )
manager.addSensor(kick_bass_sensor)
drum_tom_sensor = vizproximity.Sensor(vizproximity.Box([.2,.2,.2],center=[0.13,0.3,-.2]),source= DrumTomModel)
manager.addSensor(drum_tom_sensor)
drum_snare_sensor = vizproximity.Sensor(vizproximity.Box([.2,.2,.2],center=[-.14,1, .1]),source= DrumSnareModel)
manager.addSensor(drum_snare_sensor)
hi_hat_sensor = vizproximity.Sensor(vizproximity.Box([.3,.2,.2],center=[-.45,1, .1]),source= HiHatModel)
manager.addSensor(hi_hat_sensor)
cymbal_sensor = vizproximity.Sensor(vizproximity.Box([.3,.2,.2],center=[.45,1, .1]),source= CymbalModel)
manager.addSensor(cymbal_sensor)

"""def proximityTask1():
    #Play sound when crossing the sensor
    yield vizproximity.waitEnter(DrumTomModel,) TomDrum.hit(self)
...
target = vizproximity.Target(LeftStick)
